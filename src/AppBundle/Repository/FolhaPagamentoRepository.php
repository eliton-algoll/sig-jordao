...
SQL;
    ...
    public function findParticipantesProjetoFolhaPagamento($arFolhaPagamento)
    {
        ...
        $repository = $this->getEntityManager()->getRepository('AppBundle\Entity\ProjetoFolhaPagamento');
    ...
    public function findParticipantesFolhaSuplementar(ParameterBag $pb)
    {
        $qb = $this->getEntityManager()
            ->getRepository('AppBundle:ProjetoPessoa')
        ...
        if ($pb->get('folhaPagamento')) {
            $qb->andWhere(
                $qb->expr()->not(
                    $qb->expr()->exists(
                        'SELECT 1 FROM AppBundle:FolhaPagamento fp'
